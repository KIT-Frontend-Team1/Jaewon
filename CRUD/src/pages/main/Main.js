...
const Main = () => {
	console.log(mockCatsData[0]?.Comments);
	const [onModal, setOnModal] = useState(false);
	const { dispatch } = usePostStore();

	const onAddPost = (userId, content) => {
		const newPost = {
	...
	return (
		<>
			{onModal && <Modal onCloseModal={onCloseModal} onAddPost={onAddPost} />}
			<S.Wrapper>
				<S.Container>
					<S.CreateBtn onClick={onUseModal}>게시글 작성</S.CreateBtn>
					<S.ContentBox>
						<S.ContentBtns>
							<button>수정</button>
							<button>삭제</button>
